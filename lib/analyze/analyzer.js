...
Analyzer.prototype.analyze = function(filesObj, config, completionCallback) {
  ...
  if (filesObj.cssFiles) {
    ...
    analyzerSeries.colorguard = function (cb) {
      // colorguard prints out the results to the console no matter what
      // hijack the log add it back in after the task has run
      var oldLog = console.log;
      console.log = function () { /* no-op */ };

      colorguard.analyze(filesObj.cssFiles, '', function () {
        console.log = oldLog;
